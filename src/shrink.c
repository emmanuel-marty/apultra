...
static void apultra_optimize_forward(apultra_compressor *pCompressor, const unsigned char *pInWindow, const int nStartOffset, const int nEndOffset, const int nInsertForwardReps, const int nBlockFlags) {
   ...
   for (i = nStartOffset; i != (nEndOffset-1); i++) {
      ...
      for (m = 0; m < NMATCHES_PER_INDEX && match[m].length; m++) {
         ...
         if ((i + nMatchLen) > (nEndOffset - 1))
            nMatchLen = nEndOffset - 1 - i;
      ...
      if (pCompressor->intervals) {
         ...
         if (pCompressor->pos_data) {
            ...
            if (pCompressor->open_intervals) {
               pCompressor->arrival = (apultra_arrival *)malloc(nMaxWindowSize * NMATCHES_PER_ARRIVAL * sizeof(apultra_arrival));
